// VPC and subnets for the EKS cluster: one public and one private subnet per
// availability zone, a single NAT gateway for private-subnet egress (a
// reasonable cost/availability trade-off for this workload; use one NAT
// gateway per AZ instead if cross-AZ NAT resilience is required), and the
// "kubernetes.io/*" tags the AWS Load Balancer Controller and EKS use for
